...
- **Fase:** 3 — **en curso** (bloque de preparación). **`fase-03-filtro` (A2.1) CERRADO ✔** y **`fase-03-preflight` (A2.2) CERRADO ✔ (2026-09-25)**, ambos con verificación **PASA**; detalle en `_fases/fase-03-filtro/change-doc.md` y `_fases/fase-03-preflight/change-doc.md`. *(Fase 2 cerrada el 2026-09-23.)*
...
- **Siguiente acción:** 1) **presentar al tutor los hitos H1 (Fase 1) y H2 (Fase 2)**, aún pendientes; 2) seguir con la **preparación de la Fase 3**: **A2.2** (pre-flight anti-enmascaramiento), **A2.3** (Atomic Red Team) y **A2.4** (cronómetro `t0`/`t1`) — **un bloque por plan, cada uno con su gate humano**; después, **los ataques**.

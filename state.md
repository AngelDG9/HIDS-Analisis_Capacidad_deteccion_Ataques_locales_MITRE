...
- **Fase:** 3 — **en curso**. **Preparación CERRADA** (`fase-03-filtro`, `fase-03-preflight`, `fase-03-atomic`) y **PILOTO DE ATAQUES CERRADO ✔ (2026-09-26)**: 3 ataques × 2 iteraciones. Detalle en `_fases/fase-03-piloto/change-doc.md`.
...
- **Siguiente acción:** 1) **presentar al tutor los hitos H1 y H2**, aún pendientes; 2) **ESCALAR los ataques** — las **10 técnicas restantes** del corpus, reutilizando el runbook del piloto (`Soporte/Ataques/piloto_procedimiento.md`) y **aplicando las mejoras anotadas H1–H4**; 3) después, **Fase 4** (precio de la detección: ataques **representativos**, **sin instrumentar la Fase 3**).
- **⚠️ Mejoras anotadas del piloto (aplicar al escalar):** **H1** el pre-flight **no cubre base-contra-base** (el hallazgo `92600`); **H2** las sanidades del criterio de doble iteración se disparan con el **ruido de arranque**; **H3** el `sin_campos` genera **dudosas sistemáticas** (sesiones del operador + SCA `19004`); **H4** las señales esperadas son **genéricas** (solo nombre de proceso).

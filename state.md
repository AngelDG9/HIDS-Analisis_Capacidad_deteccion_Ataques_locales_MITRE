...
- **Fase:** 2 — Laboratorio Wazuh (F-02), plan **v2 aprobado**. Tareas **2.3 a 2.9 hechas** y **verificadas**; **2.10 a medias (1 de 2 ventanas)**.
- **Paso:** **✅ VENTANA 1 del baseline CERRADA, EXTRAÍDA Y VERIFICADA** (PASA). `t0=2026-09-23T00:45:00Z` → `t1=2026-09-23T04:45:00Z` (240 min, 48 ciclos, sin fallos). Resultado: **6.837 alertas, 12 `rule.id` distintos, 0 UNKNOWN** · RS1=46 (5 reglas) · RS2=6.791 (7 reglas) · RS3/RS4=0. **Hallazgo: el 54% del ruido es auto-ruido del propio HIDS** (`80791` = `wazuh-agentd` reescribiendo su estado cada ~5 s: 88,9%; `80792` incluye hijos de `wazuh-syscheckd`). El mantenimiento diario (~04:25Z) **no tiene firma propia**.
- **Siguiente acción (2026-09-23, tarde):** **VENTANA 2** de 4 h a las **14:00→18:00 local** (`12:00Z`→`16:00Z`), revirtiendo antes `victima-linux` a `lab-listo` y usando **el mismo script** (homogeneidad para poder comparar). Después: comparar las dos ventanas, agregar el catálogo y cerrar la fase (**2.11/2.12** + hito **H2**).
- **Pendiente menor:** corregir en `Dataset/Legitimo/baseline_meta.md` §10.4 la atribución del 2º foco de auto-ruido (es **`wazuh-syscheckd`**, no un `execve` de `wazuh-agentd`).

...
| 7 | CERRAR | Escribe `change-doc.md`; **archiva** `plan.md` y el `change-doc` en `_fases/<bloque>/`; actualiza `state.md`, `roadmap.md` y `Bitacora/`. |
...
- `plan.md` (en la raíz mientras el **bloque** corre), `reviews.md` (opcional) y `change-doc.md` —
  artefactos por **bloque**; al cerrarlo se archivan juntos en `_fases/<bloque>/` (`fase-NN` o
  `fase-NN-<nombre>`). **La unidad es el plan**: un plan por bloque; las **tandas son tareas**.
